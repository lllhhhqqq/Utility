...
public:
    ...
    auto select(const FuncT &f) const 
        -> Enumerable<typename DeclareType<decltype(f(*(T*)0))>::Type>
    {
        //functional<T>
        ...
        return Enumerable<RType>(
            [iter, end, f]() mutable
        {
            return (iter == end) 
                ? std::make_pair(false, RType())
                : std::make_pair(true, f(*iter++));
               
            /*if (iter == end) 
                return std::make_pair(false, RType());
    ...
    template<typename FuncT>
    auto where(const FuncT& f) const 
        -> Enumerable
    {
        auto iter = 
            this->begin(), 
    ...
    template<typename FuncT>
    bool all(const FuncT& f) const
    {
        bool ret = true;

        for (auto i : *this) //i in m_v
        {
            if (!f(i))
            {
                ret = false; 
    ...
    auto cast() const -> Enumerable<DestT>
    {
        ...
        {
            return (iter == end) ? std::make_pair(false, DestT()) : std::make_pair(true, DestT(*iter++));
    ...
    /*template<typename FuncT>
    auto count(const FuncT& f, 
        typename std::enable_if<!std::is_convertible<FuncT, T>::value>::type* = 0) const -> int*/
    template<typename FuncT>
    int count(const FuncT& f,
        typename std::enable_if<!std::is_convertible<FuncT, T>::value>::type* = 0) const
    {
        int n = 0;
        for (auto i : *this) {
            if (f(i)) ++n;
