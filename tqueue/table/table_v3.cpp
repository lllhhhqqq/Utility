...
struct str_from_buff {
	...
	inline size_t operator()(char **s) {
		...
		strncpy_s(str_, l+1, (buff_+sizeof(uint32_t)), l); str_[l]=0;
		*s = table_strdup(str_);
		buff_ += (l+sizeof(uint32_t));
		delete [] str_;
		return l+sizeof(uint32_t);
...
unsigned char nan[8] = {0x7f,0xff,0xff,0xff,0xff,0xff,0xff,0xff};
double table_t::cast_double(size_t row, size_t col) const
{
	const double dnan = *(double *)nan;
	double ret = dnan;
	size_t dcoltype = col_type(col);
	switch(dcoltype) {
		case tft_int: case tft_short:
			ret = get_int(row, col);
			break;
		case tft_int64:
			ret = (double)get_int64(row, col);
			break;
		case tft_uint: case tft_ushort:
			ret = get_uint(row, col);
			break;
		case tft_uint64:
			ret = (double)get_uint64(row, col);
			break;
		case tft_float:
			ret = get_float(row, col);
			break;
		case tft_double:
			ret = get_double(row, col);
			break;
		case tft_str:
			ret = atof(get_str(row, col));
			if (0x0000000000000000 == ret)
			{
				ret = dnan;
			}
			break;
		...
		case tft_char: case tft_uchar:
			{
				size_t len = col_size(col);
				char * str = new char[len+1];
				memcpy(str, get_buff(row, col), len);	str[len]=0;
				ret = atof(str);
				delete [] str;
				if (0x0000000000000000 == ret)
				{
		...
		case tft_pfloat:
		case tft_vfloat:
			ret = (double)get_int64(row, col);  //同 table_getint64, 设置和返回 int64 即可
			break;
		default:
			;//assert(false);
	}
...
long long table_t::cast_int64(size_t row, size_t col) const
{
	long long ret = 0;
	size_t dcoltype = col_type(col);
	switch(dcoltype) {
	case tft_int: case tft_short:
		...
		break;
	case tft_int64:
		ret = get_int64(row, col);
		break;
	case tft_uint: case tft_ushort:
		ret = get_uint(row, col);
		break;
	case tft_uint64:
		ret = get_uint64(row, col);
		break;
	case tft_float:
		ret = (long long)get_float(row, col);
		break;
	case tft_double:
		ret = (long long)get_double(row, col);
		break;
	case tft_str:
		ret = atoi(get_str(row, col));
		break;
	case tft_table:
		break;
	case tft_char: case tft_uchar:
	...
	case tft_pfloat:
	case tft_vfloat:
		ret = get_int64(row, col);  //同 table_getint64, 设置和返回 int64 即可
		break;
	default:
		;//assert(false);
	}
